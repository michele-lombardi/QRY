...
- `audit-privacy.mjs`: runs the cross-platform static part of the privacy audit;
- `audit-release-content.mjs PATH...`: rejects local databases, CSV exports,
  environment files and logs in content selected for packaging;
- `audit-release-assets.mjs DIRECTORY VERSION`: checks the exact macOS/Windows
  release asset set and independently verifies every SHA-256 file;
...
- `package-macos.sh TARGET`: builds the ad-hoc-signed app, verifies its bundle
  ...
  `x86_64`;
- `package-windows.ps1 -Target x86_64-pc-windows-msvc`: verifies existing NSIS
  and MSI outputs, normalizes their public names and creates SHA-256 files;
